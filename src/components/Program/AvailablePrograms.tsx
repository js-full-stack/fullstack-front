...
const AvailablePrograms = observer(() => {
  const programsStore = useStores().program;
  const authStore = useStores().auth;

  const avialablePrograms = programsStore.programs.filter(
    (program) => program.isSubscribe === false
  );

  return (
    <>
      {avialablePrograms.length > 0 ? (
        <h2>Total available programs: {avialablePrograms.length}</h2>
      ) : (
        <p>No programs available</p>
      )}
      {authStore.currentUser?.role === "athlete" &&
        avialablePrograms.map(
          ({
            name,
            id,
            description,
            ...
            <div key={id}>
              ...
              <Button
                onClick={() => programsStore.subscribeUserToProgram(id)}
                style={styles.button}
                variant="warning"
              >
                Subscribe
